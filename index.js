...
// TODO: Clean up the global variable 
var chatId = "";
...
class FlatData {
  constructor(flatBlock, flatNumber, society) {
...
var splitInput = function (inputText) {
  ...
  var society = "ACZ"; // Defaulted to ACZ
  var flatBlock = "";
  if (splitArr.length >= 2) {
    // TODO: Remove this check so that it works for other Aparna societies
    if (splitArr[1].toUpperCase() != "ASZ" && splitArr[1].toUpperCase() != "ACZ") {
      var errorMsg = "Supported Societies ACZ & ASZ. Apartment Input given is " + splitArr[1];
      debugLog(errorMsg);
  ...
  var flatBlockSplit = flatBlock.split("-");
  if (flatBlockSplit != null && flatBlockSplit.length == 2) {
    return new FlatData(flatBlockSplit[0], flatBlockSplit[1], society);
  } else {
...
var postPTINDataBack = function (ptinDataObject) {
  ...
  stringOutput +=
    "\n <a href='https://ptghmconlinepayment.cgg.gov.in/SearchYourProperty.do?method=getDues&i_asmtno=" +
    ptinDataObject[1] +
    "'>More Details</a>"; // append the PTIN number.
  // TODO: gather more details and display the dues, due dates and total dues.
  sendMessage(stringOutput);
};

var loopAndReturnCorrectPTINObject = function (
  blockName,
  flatNumber,
  dataArray
) {
...
var retrievePTINData = function (blockName, flatNumber, data) {
  try {
    ...
    if (data.iTotalRecords == 1) {
      ...
      var searchedResult = loopAndReturnCorrectPTINObject(
        blockName,
        flatNumber,
        data.aaData
      );
